...
touch $ROOT_CA/db/networkDSB-ca.db
touch $ROOT_CA/db/networkDSB-ca.db.attr
...
touch $ROOT_CA_INTERMEDIATE/db/networkDSB-ca.db
touch $ROOT_CA_INTERMEDIATE/db/networkDSB-ca.db.attr
...
#  Create the root key
#Encrypt the root key with AES 256-bit encryption and a strong password.
#openssl genrsa -aes256  -out $ROOT_CA_INTERMEDIATE/private/networkDSB.key.pem 2048
openssl genrsa \
  -out $ROOT_CA_INTERMEDIATE/private/networkDSB.key.pem \
  2048
# chmod 400 $ROOT_CA_INTERMEDIATE/private/networkDSB.key.pem


openssl req -config $CONF_DIR_INTERMEDIATE/openssl.cnf \
  -key $ROOT_CA_INTERMEDIATE/private/networkDSB.key.pem \
  -new -sha256 -out $ROOT_CA_INTERMEDIATE/csr/networkDSB.csr.pem

#Create  server a certificate
# sign the CSR
openssl ca -config $CONF_DIR_INTERMEDIATE/openssl.cnf -batch \
  -extensions server_cert -days 375 -notext -md sha256 \
  -in $ROOT_CA_INTERMEDIATE/csr/networkDSB.csr.pem \
  -out $ROOT_CA_INTERMEDIATE/certs/server.networkDSB.cert.pem
# chmod 444 $ROOT_CA_INTERMEDIATE/certs/server.networkDSB.cert.pem

openssl x509 -noout -text \
  -in $ROOT_CA_INTERMEDIATE/certs/server.networkDSB.cert.pem
...
openssl ca -config $CONF_DIR_INTERMEDIATE/openssl.cnf -batch \
  ...
  -in $ROOT_CA_INTERMEDIATE/csr/networkDSB.csr.pem  \
  -out $ROOT_CA_INTERMEDIATE/certs/client.networkDSB.cert.pem
# chmod 444 $ROOT_CA_INTERMEDIATE/certs/client.networkDSB.cert.pem
...
# create fullchain
cat $ROOT_CA_INTERMEDIATE/certs/server.networkDSB.cert.pem \
...
cp -r $ROOT_CA/certs/ca.cert.pem $ROOT_DIR/ca/networkDSB-root-ca.crt.pem
cp -r $ROOT_CA/private/ca.key.pem $ROOT_DIR/ca/networkDSB-root-ca.key.pem
...
cp -r $ROOT_CA_INTERMEDIATE/private/networkDSB.key.pem $ROOT_DIR/server/privkey.pem
cp -r $ROOT_CA_INTERMEDIATE/certs/server.networkDSB.cert.pem $ROOT_DIR/server/cert.pem
...
openssl verify -CAfile $ROOT_CA_INTERMEDIATE/certs/ca-chain.cert.pem \
  $ROOT_CA_INTERMEDIATE/certs/server.networkDSB.cert.pem

# verify server
openssl verify -CAfile $ROOT_CA_INTERMEDIATE/certs/ca-chain.cert.pem \
  $ROOT_CA_INTERMEDIATE/certs/client.networkDSB.cert.pem

openssl rsa -noout -modulus -in $ROOT_DIR/server/privkey.pem
openssl req -noout -modulus -in $ROOT_CA_INTERMEDIATE/csr/networkDSB.csr.pem
